# TODO: create a transport class of our own and emulate all functions used here
# TODO: add a unique id attribute and pass when sending using trasport layer
...
class SyncObj(object):
    def __init__(self, 
        ...
        # self.__transport.setOnMessageReceivedCallback(self.__onMessageReceived)    #done
        # self.__transport.setOnReadonlyNodeConnectedCallback(self.__onReadonlyNodeConnected)         #dont care
        # self.__transport.setOnReadonlyNodeDisconnectedCallback(self.__onReadonlyNodeDisconnected)   #dont care
            ...
            if self.__raftState == _RAFT_STATE.LEADER:
                ...
                if changeClusterRequest is None or self.__changeCluster(changeClusterRequest):
                    ...
                    if requestNode is None:
                        ...
                        self.__send(self.__uid,requestNode, {
                ...
                if requestNode is None:
                    ...
                    self.__send(self.__uid,self.__raftLeader, message)
                else:
                    self.__send(self.__uid,requestNode, {
                        'type': 'apply_command_response',
                        'request_id': requestID,
        ...
        if self.__raftState == _RAFT_STATE.LEADER:
            ...
            for node in self.__otherNodes:
                if self.__lastResponseTime[node] > deadline:
                    count += 1#specify config parameters
        #journal file also specify
            if count <= (len(self.__otherNodes) + 1) / 2:
                ...
                if nextNodeIndex > self.__raftLog[0][1]:
                    ...
                    if len(entries) == 1 and len(entries[0][0]) >= batchSizeBytes:
                        ...
                        for pos in xrange(0, len(entry), batchSizeBytes):
                            ...
                            self.__send(self.__uid,node, message)
                            if node not in self.__connectedNodes:
                                break
                    else:
                        message = {
