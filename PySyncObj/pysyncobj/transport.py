...
class TCPTransport(Transport):
    ...
    def _onNewIncomingConnection(self, conn):
        ...
        self._unknownConnections.add(conn)          # Add to the list of unknown connections
        encryptor = self._syncObj.encryptor 
        if encryptor:
            conn.encryptor = encryptor
        conn.setOnMessageReceivedCallback(functools.partial(self._onIncomingMessageReceived, conn)) # A new function with the conn argument frozen
        conn.setOnDisconnectedCallback(functools.partial(self._onDisconnected, conn))  # A new function with the conn argument frozen
